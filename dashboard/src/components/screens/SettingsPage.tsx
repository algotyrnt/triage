...
export const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const [newlyCreatedKeyModal, setNewlyCreatedKeyModal] = useState<{
    name: string;
    rawKey: string;
  } | null>(null);
  ...
  const handleCreateKey = async () => {
    ...
    try {
      ...
      if (res.success && res.key) {
        ...
        if (raw) {
          setNewlyCreatedKeyModal({ name: res.key.name, rawKey: raw });
        ...
        <div className="lg:col-span-9 space-y-6">
          ...
          {activeTab === 'keys' && (
            <div className="bg-white border border-slate-200 rounded-sm p-4 space-y-4 font-mono text-xs">
                ...
                <div className="space-y-3">
                  {keysList.map((key) => {
                    return (
                      <div
                        key={key.id}
                        className="border border-slate-200 bg-slate-50/50 rounded-sm p-3 space-y-2 transition-colors"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center justify-between gap-2 bg-white p-2.5 rounded-sm border border-slate-200 text-xs">
                          <div className="flex items-center gap-2">
                            <span className="text-slate-400 font-mono text-[11px]">Key:</span>
                            <code className="text-slate-800 font-bold tracking-wide font-mono">
                              ••••••••••••••••••••••••{key.keyMasked.replace('...', '')}
                            </code>
                          </div>
                          {key.status === 'ACTIVE' && (
                            <button
                              onClick={() => handleRevokeKey(key.id)}
                              className="text-red-600 hover:text-red-800 text-[11px] font-mono underline cursor-pointer shrink-0"
                            >
                              Revoke
                            </button>
                          )}
              ...
              {newlyCreatedKeyModal && (
                <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
                  <div className="bg-white border border-slate-200 rounded-lg max-w-md w-full p-6 space-y-4 shadow-2xl">
                    ...
                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-sm text-xs text-amber-900 space-y-1">
                      <div className="font-bold flex items-center gap-1.5 text-amber-800">
                        ...
                        <span>Save this key securely</span>
                      </div>
                      <p className="text-[11px] leading-relaxed text-amber-800 font-sans">
                        Make sure to copy your API key now. For your security, you will not be able
                        to see it again.
                      </p>
                    </div>

                    <div className="space-y-2">
                      <div className="bg-slate-900 p-3 rounded-sm text-xs font-mono text-emerald-400 border border-slate-800 break-all select-all font-bold">
                        {newlyCreatedKeyModal.rawKey}
                      </div>
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(newlyCreatedKeyModal.rawKey);
                          setKeyToast({
                        ...
                        className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs py-2 rounded-sm flex items-center justify-center gap-2 transition-colors cursor-pointer"
                      >
                    ...
                    <div className="pt-2 border-t border-slate-100 flex justify-end">
                      <button
                        onClick={() => setNewlyCreatedKeyModal(null)}
                        className="bg-black hover:bg-slate-800 text-white text-xs font-bold px-4 py-2 rounded-sm cursor-pointer font-mono"
                      >
                        Done
                      </button>
                    </div>
