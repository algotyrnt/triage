...
func TestMiddlewareWithOptions(t *testing.T) {
	...
	mw := Middleware(
		...
		WithRootPath("backend/services/api"),
	)
	handler := mw(panicHandler)

	req := httptest.NewRequest("GET", "/options-test", nil)
	rec := httptest.NewRecorder()
	...
	case payload := <-telemetryChan:
		...
		if !strings.Contains(strPayload, `"commit":"a1b2c3d4e5f6"`) {
			t.Errorf("expected commit SHA in payload, got %s", strPayload)
		}
		if !strings.Contains(strPayload, `"owner":"algotyrnt"`) || !strings.Contains(strPayload, `"repo":"triage"`) {
			t.Errorf("expected owner and repo in payload, got %s", strPayload)
		}
		if !strings.Contains(strPayload, `"root_path":"backend/services/api"`) {
			t.Errorf("expected root_path in payload, got %s", strPayload)
		}
	case <-time.After(3 * time.Second):
		t.Fatalf("timeout waiting for telemetry payload with options")
	}
}

func TestMiddlewareWithMonorepoRootPath(t *testing.T) {
	telemetryChan := make(chan []byte, 1)
	...
	panicHandler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		panic("monorepo service panic")
	})

	mw := Middleware(
		"tr_live_monorepo_key",
		ts.URL,
		WithRepo("algotyrnt/my-monorepo"),
		WithRootPath("/backend/"), // should trim leading and trailing slashes
	)
	handler := mw(panicHandler)

	req := httptest.NewRequest("GET", "/monorepo", nil)
	rec := httptest.NewRecorder()
	...
	case payload := <-telemetryChan:
		strPayload := string(payload)
		if !strings.Contains(strPayload, `"root_path":"backend"`) {
			t.Errorf("expected trimmed root_path 'backend' in payload, got %s", strPayload)
		}
		if !strings.Contains(strPayload, `"owner":"algotyrnt"`) || !strings.Contains(strPayload, `"repo":"my-monorepo"`) {
			t.Errorf("expected owner and repo in payload, got %s", strPayload)
		}
	case <-time.After(3 * time.Second):
		t.Fatalf("timeout waiting for monorepo telemetry payload")
	}
}
